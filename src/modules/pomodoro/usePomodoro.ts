...
import { PomodoroAction, PomodoroConfig, PomodoroState, PomodoroType } from "./types";
...
const changeConfigAction = (payload: Partial<PomodoroConfig>): PomodoroAction => ({
  type: "changeConfig",
  payload,
});
...
const formatTime = (timeInSeconds: string | number) => {
  ...
  const parsedTimeInSeconds =
    typeof timeInSeconds === "string" ? parseInt(timeInSeconds, 10) : timeInSeconds;
...
export const usePomodoro = (config: Partial<PomodoroConfig> = defaultConfig) => {
  ...
  const toggle = useMemo(() => (rawState.paused ? start : stop), [rawState.paused]);
  ...
  const goShortBreak = useCallback(() => changeType("shortBreak"), [changeType]);
  const goLongBreak = useCallback(() => changeType("longBreak"), [changeType]);

  // timer setup
  useInterval(tick, rawState.paused ? null : 1000);

  const formattedTimer = useMemo(() => formatTime(rawState.timer), [rawState.timer]);
  ...
  const progressInPercent = useMemo(() => (progress * 100).toFixed(2) + "%", [progress]);
  ...
  const nextType: PomodoroType = useMemo(() => {
    ...
    if (type === "pomodoro" && pomodoros > 0 && (pomodoros + 1) % config.longBreakInterval === 0) {
      return "longBreak";
    }
  ...
  const next = useCallback(() => dispatch(nextAction(nextType)), [dispatch, nextType]);
...
export const PomodoroProvider: FC<PomodoroProviderProps> = ({ children, config }) => {
